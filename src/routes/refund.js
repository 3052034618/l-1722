...
router.get('/', async (req, res) => {
  try {
    ...
    res.json({ code: 200, message: '获取退票/改签记录成功', data: records });
  } catch (err) {
...
router.put('/:id/approve', async (req, res) => {
  try {
    const { processedBy, newSeatIds } = req.body;
    ...
    if (refundRecord.type === 'refund') {
      result = await RefundService.approveRefund(req.params.id, processedBy);
    } else {
      if (!newSeatIds || newSeatIds.length === 0) {
        return res.status(400).json({ code: 400, message: '改签审批需要提供新座位', data: null });
      }
      result = await RefundService.approveReschedule(req.params.id, processedBy, newSeatIds);
    }
...
router.put('/:id/reject', async (req, res) => {
  try {
    ...
    const result = await RefundService.rejectRequest(req.params.id, processedBy);
